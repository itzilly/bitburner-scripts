...
function rename_server(ns, old_name, new_name) {
    if (old_name === undefined || new_name === undefined) {
        ns.tprintf("Please select which server you'd like to rename\n");
        show_help(ns, 'rename')
        return
    }
    ns.renamePurchasedServer(old_name, new_name)
    ns.tprintf(`Renamed: ${old_name} -> ${new_name}`)
    return
}
...
function show_help(ns, help_message, should_exit = true) {
    ...
    ns.tprintf('=----=----=----=')
    ns.tprintf('PServer Manager')
    ns.tprintf('=----=----=----=')
    ns.tprintf('args:')

    if (helps[help_message] === undefined) {
        for(const key of Object.keys(helps)) {
            ns.tprintf(`       ${helps[key]}`)
        }
    }
    else { ns.tprintf(`      ${helps[help_message]}`) }
